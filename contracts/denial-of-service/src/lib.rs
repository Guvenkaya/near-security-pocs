...
impl Display for MoneyJar {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "MoneyJar: {}", self.amount.0)
    }
}
...
impl Contract {
    ...
    #[private]
    pub fn add_reward_to_each_jar(
        &mut self,
        account_ids: Vec<(AccountId, U128)>,
    ) {
        for (account_id, reward) in account_ids {
            let jars = self
                .jars_per_user
                .get_mut(&account_id)
                .unwrap_or_else(|| env::panic_str("No jars found for account"));

            for mut jar in jars.iter().cloned() {
                jar.amount.0 += reward.0;
                log!("Adding reward to jar: {}", jar.amount.0);
            }
        }
    }

    #[private]
    pub fn add_reward_to_each_jar_log_size_limit(
        &mut self,
        account_ids: Vec<(AccountId, U128)>,
    ) {
        let mut processed_jars = Vec::new();
        ...
        for (account_id, reward) in account_ids {
            ...
            for mut jar in jars.iter().cloned() {
                jar.amount.0 += reward.0;
                processed_jars.push(jar);
            }
        }

        log!("Processed jars: {:#?}", processed_jars);
    }

    pub fn claim_all_jars(&mut self) {
        let account_id = env::predecessor_account_id();
        let jars = self
            .jars_per_user
            .get_mut(&account_id)
        ...
        let total_amount = jars
            .iter()
            .cloned()
            .map(|mut jar| {
                let amount = jar.amount.0;
        ...
        Promise::new(account_id)
            .transfer(NearToken::from_yoctonear(total_amount));
    }

    pub fn create_jar(&mut self, amount: U128, id: U128) {
        let account_id = env::predecessor_account_id();

        let jar = MoneyJar::new(amount, id.clone());
    ...
    pub fn batch_create_jars(&mut self, jars: Vec<(U128, U128)>) {
        let account_id = env::predecessor_account_id();

        for (amount, id) in jars {
            let new_id = U128(id.0 + env::block_timestamp_ms() as u128);

            let jar = MoneyJar::new(amount, new_id);
        ...
        log!("Created jars for user: {}", account_id);
    }
